...
\lstset{language=python,
	  	basicstyle=\ttfamily,
        	        ...
        	        commentstyle=\color{RoyalPurple}\ttfamily,
...
\lstset{caption={Fonction check\_defined\_rules}, captionpos=b} 
\begin{lstlisting}
...
\item S'il existe une suite $U_n$ qui dénombre les objets de la
  ...
  EqualGram : \url{https://oeis.org/A126869}\\
...
\lstset{caption={Exemple}, captionpos=b} 
\begin{lstlisting}
...
\lstset{caption={Grammaire des arbres}, captionpos=b} 
\begin{lstlisting}
treeGram = {
"Tree" : CR.Union(
...
\begin{itemize}
\item $Bound.count() = \sum\limits_{i = min}^{max}\ r.count(i)$
\item $Bound.list () = \sum\limits_{i = min}^{max}\ r.list(i)$ (ici,
  $\sum$ correspond à la concaténation de listes)
\item  $Bound.unrank(n) = r.unrank(j, n - $\sum\limits_{i = min}^{j-1}
  count(i)$) avec j le plus petit entier tel que $\sum\limits_{i = min}^j count(i) \geq n$
\item $Bound.rank(obj) = \sum\limits_{i = min}^{len(obj)}\ r.count(i)
  + r.rank(obj)\ avec\ len(obj)<max$
\item $Bound.random() = Bound.unrank(random(0, Bound.count))$
\end{itemize}

\section {Sequence}

Le constructeur Sequence a été ajouté aux grammaires condensées dans
le fichier CRules.py. Il prend en argument un non terminal (chaine de
caractères), un cas vide (objet), un constructeur (fonction), et
optionellement deux fonctions pour rank (une pour l'union et une pour
le produit). La fonction to\_simple\_rule effectue la traduction de la
règle telle que décrite dans l'énoncé.\\

On réécrit la grammaire DyckGram en utilisant le nouveau constructeur:
\lstset{caption={Grammaire des mots bien parenthésés}, captionpos=b} 
\begin{lstlisting}
DyckGram = {
