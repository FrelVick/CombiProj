...
# Fonction de caching pour unrank
def memoizeu (f):
    memo = {}
    def helper(s, n, i):
...
# Fonction de caching pour count
def memoizec (f):
    memo = {}
    def helper(s, i):
        if (s, i) not in memo:
...
''' Fonction de caching pour rank, suppose
que les objets définissent la fonction __str__'''
def memoizer (f):
    memo = {}
    def helper(s, o):
        ''' On utilise str(o) plutot que de définir une fonction de hachage.
        On suppose donc que deux objets égaux sont représentés par la meme chaine 
        de caractères et que deux objets différents sont représentés par des chaines
...
class EpsilonRule(ConstantRule) :
    ...
    def valuation(self):
        ''' Retourne la valeur de la valuation ''' 
    ...
    def list(self, i):
        ''' Retourne la liste vide (si i < 0 ou i > 0) ou la liste composée du seul objet
        produit par la règle Epsilon '''
        if i < 0:
    ...
    @lru_cache(maxsize=1000)
    def unrank(self, n, i):
        ''' Unrank pour EpsilonRule est défini uniquement pour i, n = 0'''
        if i == 0 and n == 0 :
            return self._object
    ...
    @lru_cache(maxsize=1000)
    def rank(self, obj):
        ''' Rank pour EpsilonRule est défini uniquement pour l'objet passé en argument
        au constructeur '''
        if obj == self._object:
            return 0
        else:
    ...
    def random(self, n):
        # Un seul objet produit par EpsilonRule
        return self._object
...
class SingletonRule(ConstantRule) :
    ...
    @lru_cache(maxsize=1000)
    def count(self, i):
        ''' La règle Singleton ne produit qu'un seul objet qui est de poids 1 '''
        if i < 0 : 
    ...
    def list(self, i):
        ''' Retourne la liste vide (si i différent de 1) ou la liste composée du seul objet
        produit par la règle Singleton '''
        if i < 0:
    ...
    @lru_cache(maxsize=1000)
    def unrank(self, n, i):
        ''' Unrank pour EpsilonRule est défini uniquement pour i = 0,  n = 1'''
        if i == 0 and n == 1:
            return self._object
    ...
    @lru_cache(maxsize=1000)
    def rank(self, obj):
        ''' Rank pour SingletonRule est défini uniquement pour l'objet passé en argument
        au constructeur '''
        if obj == self._object:
            return 0
        else:
    ...
    def random(self, n):
        # Un seul objet produit par SingletonRule
        return self._object
...
class ConstructorRule(AbstractRule):
    ...
    # Retourne une paire composée des sous règles de la règle
...
class UnionRule(ConstructorRule):
    ...
    def rank(self, obj):
        ...
        if self._derive_from_first(obj):
            # L'objet dérive de la première règle
            return fst.rank(obj)
        else:
            # L'objet dérive de la seconde règle, il faut ajouter
            # le nombre d'objet créés par la première règle
...
class ProductRule(ConstructorRule):
    ...
    def count(self, i):
        ...
        v2 = r2.valuation()
        res = 0
        for i1 in range (v1, i + 1): # correspond à k >= V(N1)
            if (i - i1 >= v2):  # correspond à l >= V(N2)
    ...
    def unrank(self, n, i):
        ...
        for j in range(n+1):
            ...
            if (i < total):
                # On a trouvé les tailles des objets générés par les sous règles
                # On peut appliquer récursivement unrank
...
class Bound:
    ...
    def __init__(self, rule, min, max):
        self._rule = rule
        if min < max :
            raise ValueError("BOUND : lower bound must be smaller than upper bound")
        self._min = min
        self._max = max

    # Retourne la somme du nombres d'objets de taille min, min + 1, ..., max-1
    @lru_cache(maxsize=1000)
    def count(self):
        count = 0
    ...
    # Retourne la concaténation des listes d'objets de taille min, min + 1, ..., max-1
    def list(self):
        l = []
        for i in range(self._min, self._max):
            l += self._rule.list(i)
    ...
    # Retourne l'objet de rang i dans l'ensemble des objets de taille appartenant à [min, max - 1]
    @lru_cache(maxsize=1000)
    def unrank(self, i):
        if (i < 0):
        ...
        for j in range(self._min, self._max):
            ...
            # On a trouvé la taille de l'objet
            if (i < rc + count):
    ...
    # Retourne l'objet de rang de obj dans l'ensemble des objets de taille appartenant à [min, max - 1]
    @lru_cache(maxsize=1000)
    def rank(self, obj):
        if (len(obj) > self._max):
            raise ValueError("Bound rule RANK : object not in range (min, max)")
        rank = 0
        # On calcule 
    ...
    def random(self):
        return self.unrank(random.randrange(self.count()))
